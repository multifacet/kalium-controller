...
class SSLSocket(socket):
	...
	def send(self, data, flags=0):
		...
		session_id = self.get_channel_binding().encode("hex")
		...
		if method == "POST":
			tmp = data.split("\r\n")[0]
			url = tmp.strip("\r").strip("\n").split(" ")[1]
			has_body = 1
		
		msg = "%s:%s:%s:%s:%s:%s" %  (self.server_hostname + url, method, ip, port, has_body, session_id)


		buf = json.dumps({"meta": msg, "data": data})
		resp = send_event(EVENT_SEND, buf)

		if resp == os.environ['AWS_LAMBDA_FUNCTION_NAME']:
			...
			if kw1 in data:
				...
				data = data[: idx1] + new_header + data[idx1: ]
	...
	def recv(self, buflen=1024, flags=0):
		#########################################
		## Ignore the changes to this function ##
		#########################################
		session_id = self.get_channel_binding().encode("hex")
		ip, port = self._sock.getpeername()
		msg = "%s:%s:%s:%s" %  (self.server_hostname, ip, port, session_id)
		...
		if self._sslobj:
			...
			resp = os.environ['AWS_LAMBDA_FUNCTION_NAME']
			try:
				header, body = buf.split("\r\n\r\n")[0], buf.split("\r\n\r\n")[1]
			except:
				header = ""
				body = buf
			...
			try:
				...
				pad = len(body) - len(new_body)
				# print "pad", pad
				# print header, len(header)
				# print body, len(body)
				# print new_body, len(json.dumps(new_body))
				
				if len(buf) == len(body):
					buf = "%s%s" % (new_body, ' ' * pad)
				else:
					buf = "%s\r\n\r\n%s%s" % (header, new_body, ' ' * pad)


				tmp = json.dumps({"meta": msg, "data": len(buf)})
				resp = send_event(EVENT_RESP, tmp)
				# print "recv", resp
				# print header
				if resp == os.environ['AWS_LAMBDA_FUNCTION_NAME']:
					pass
			...
			buf = self._sock.recv(buflen, flags)
			tmp = json.dumps({"meta": msg, "data": len(buf)})
			resp = send_event(EVENT_RESP, tmp)
			# print "recv", resp, buf
			if resp == os.environ['AWS_LAMBDA_FUNCTION_NAME']:
